...
fn get_jwt_secret() -> String {
    std::env::var("JWT_SECRET").unwrap_or_else(|_| "dev-secret-key-verity-2025".to_string())
}
...
pub async fn get_nonce(
    ...
    state.db.save_nonce(&nonce).await?;
...
pub async fn verify_and_login(
    ...
    tracing::info!("Verifying signature for address: {}", payload.address);
    ...
    // Parse signature
    let signature = PrimitiveSignature::from_str(&payload.signature)
    ...
    tracing::info!("Recovered address: {}", recovered_address);

    // Verify recovered address matches provided address
    if recovered_address.to_string().to_lowercase() != payload.address.to_lowercase() {
        return Err(anyhow::anyhow!("Signature verification failed").into());
    }

    // Get or create user
    let user = match state.db.get_user_by_address(&payload.address).await? {
        Some(existing_user) => {
            tracing::info!("User found: {:?}", existing_user.id);
            existing_user
        }
        None => {
            tracing::info!("Creating new user for address: {}", payload.address);
            state.db.create_user(&payload.address).await?
        }
    };
    ...
    let _wallet = match state
        .db
        .get_wallet_by_address(payload.address.clone())
        .await?
    {
        Some(existing_wallet) => existing_wallet,
        None => state.db.create_wallet(&user_id, &payload.address).await?,
    };

    // Generate JWT
    let token = generate_jwt(user_id.clone())?;
    tracing::info!("Authentication successful for user: {}", user_id);
